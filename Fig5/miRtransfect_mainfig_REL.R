...
df.n = df.processed %>%
  ungroup() %>%
  filter(!excluded) %>%
  count(expeID, cells, miR, point.class) %>%
  pivot_wider(names_from = point.class, values_from = n, values_fill = 0) %>%
  mutate(FALSE.outlier = TRUE.outlier + FALSE.outlier) %>%
  select(-TRUE.outlier)
df.n.print = df.n %>%
  mutate(total = FALSE.none + TRUE.none + FALSE.outlier + FALSE.hit + TRUE.hit) %>%
  select(-FALSE.none) %>%
  pivot_longer(cols = c(-expeID, -cells, -miR), names_to = "point.class", values_to = "counts") %>%
...
df.n.summ = df.n %>%
  pivot_longer(cols = c(-expeID, -cells, -miR), names_to = "point.class", values_to = "counts") %>%
  mutate(point.class = factor(point.class, levels = rev(point.classes.summ))) %>%
  mutate(expeID.flat = paste(cells,
                             sub("\\.", "\uad", miR),
                             sep = " "))
df.n.summ.order = df.n.summ %>%
  filter(point.class %in% c("TRUE.hit", "FALSE.hit")) %>%
  group_by(expeID.flat) %>%
  summarize_at("counts", sum) %>%
  arrange(-counts) %>%
  pull(expeID.flat)
df.n.summ = df.n.summ %>%
  mutate(expeID.flat = factor(expeID.flat, levels = df.n.summ.order)) %>%
  arrange(expeID.flat, point.class)
...
## Summary bar plot ----
df.n.summ %>%
  ggplot(aes(x = expeID.flat, y = counts, fill = point.class)) +
  geom_col(position = "fill") +
  geom_hline(yintercept = 1, color = "black", linewidth = 0.5) +
  scale_fill_manual(
    breaks = point.classes.summ,
    values = c(
      "gray80",
      "darkblue",
      "magenta2",
  ...
  labs(x = "Dataset", y = "Proportion of genes") +
  theme_plot +

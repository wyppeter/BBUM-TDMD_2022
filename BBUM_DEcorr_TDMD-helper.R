...
#
# See documentation of BBUM_DEcorr for more details: ?bbum::BBUM_DEcorr
...
# - let-7a <--> let-7a*
# - let-7a <--> let-7a_star
# - let-7a-5p <--> let-7a-3p (e.g. let-7a-3p.2 is also allowed)
#
# Outputs a tibble of the results table
...
# - FCdir.up: whether fold change went up i.e. sample set (bool)
# - tdmd.miR, tdmd.star: TDMD-sensitive miR and its star (other strand) (bool)
...
BBUM_DEcorr_TDMD = function(
  ...
  df.bbum.out = deseq_results %>%
    ...
    dplyr::mutate(
      ...
      miRNA.pre = sub("(\\-(3p|5p)(\\.[0-9])?)$|(_star)$|(\\*)$", "", miRNA)  # guide/star pair detector
    )
        ...
        dplyr::if_else(tdmd.miR,
                       ...
                       dplyr::if_else(tdmd.star,
                                      ...
                                      dplyr::if_else(outlier,
                                                     "outlier",
                                                     "none")
                       )), levels = c("none","passenger","guide","outlier"))  # summaritive factor for categories
    ) %>%
    dplyr::ungroup()
